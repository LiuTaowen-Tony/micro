from typing import List, Dict, Union, Tuple, TypedDict, Callable
import datasets
import torch
...
# DPOSample = TypedDict('DPOSample', {'prompt': str, 'chosen': str, 'rejected': str, 'chosen_response_only': str, 'rejected_response_only': str, 'chosen_input_ids': List[int], 'chosen_attention_mask': List[int], 'chosen_labels': List[int], 'rejected_input_ids': List[int], 'rejected_attention_mask': List[int], 'rejected_labels': List[int], 'prompt_input_ids': List[int], 'prompt_attention_mask': List[int], 'prompt_token_type_ids': List[int]})


class DPOPromptItem(TypedDict):
    responses: List[str]
    pairs: List[Tuple[int, int]]
    sft_target: str


def get_se(split, silent=False, cache_dir: str = None) -> Dict[str, DPOPromptItem]:
...
class DPODataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.flat_pairs = []
        self._get_flat_pairs()

    def _get_flat_pairs(self):
        for prompt, response, pairs, sft_target, truncation_mode in self.flat_data:
            for p in pairs:
                self.flat_pairs.append(
                    (prompt, response, p, sft_target, truncation_mode)
                )

    def __iter__(self):
        for idx in range(len(self.flat_pairs)):
            yield self[idx]

    def __len__(self):
        return len(self.flat_pairs)

    def __getitem__(self, idx):
        prompt, response, (chosen_idx, reject_idx), sft_target, truncation_mode = (
            self.flat_pairs[idx]
        )
        item = tokenize_one(
            prompt,
